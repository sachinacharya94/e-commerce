...
const edit = () => {
    ...
    const handleEdit = async(e)=>{
        e.preventDefault()
        await updateCategory(category,id,token)
        .then(data =>{
            if(data.error){
            ...
            <form >
                <div className="grid gap-4 mb-4 sm:grid-cols-2">
                    <div>
                        ...
                        <input type="text" name="name" id="name" className="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500" placeholder="Type category name" required="" onChange={(e)=>setCategory(e.target.value)} value={category.category_name}/>
                    </div>
